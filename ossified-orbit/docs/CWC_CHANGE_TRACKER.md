...
| FMS-001 | Forms / Branded success redirect | Ensure successful Join and Contact submissions return to the CWC `/thank-you` page. | P0 | Blocked — account setting | Formspree owner access for form IDs `xdarjdzy` (Join) and `xvkpzowb` (Contact) | In each Formspree form’s Settings, set the Thank You redirect to exactly `https://www.christianwomencommunity.org/thank-you`; submit each production form once and confirm the final URL and received submission. | Audit result: both forms previously carried a hidden `_next` value, but Formspree’s current supported redirect is its per-form Settings redirect attribute (available on Personal, Professional, and Business plans), not a source-controlled hidden field. That is why the successful Join submission used Formspree’s generic page. The Contact field additionally named a non-existent `/contact/thank-you` route. The unsupported/stale `_next` fields were removed; no client-side redirect was added. Preview submissions will intentionally redirect to the production absolute URL after the Formspree setting is applied. [Formspree redirect documentation](https://help.formspree.io/articles/form-and-project-settings/thank-you-redirect) |
| OPS-001 | Operations / Email | Activate or connect `info@christianwomencommunity.org` to the user’s email through the correct provider. | P1 | Pending — operational | Domain DNS/provider identification and owner-authorised access | Confirm the domain/DNS provider, whether a mailbox already exists, the intended recipient address for forwarding, and whether outgoing mail is required; then configure and test inbound (and outbound if used) mail. | Source audit: the address appears in `src/config/site.ts` and as the General Enquiries `mailto:` link on `/contact`; the Contact form sends to Formspree and does not itself use this address. No source evidence identifies the email/DNS provider or proves mailbox/forwarding works. Cheapest likely arrangement is domain-provider email forwarding to an existing inbox if the provider includes it; otherwise select a low-cost mailbox/forwarding service once provider access is supplied. Do not guess credentials or alter DNS. |
